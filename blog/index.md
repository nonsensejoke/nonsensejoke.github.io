...
<p>
{% for s in site.data.sections %}<a href="#{{ s.id }}">{{ s.name }}</a>{% unless forloop.last %} · {% endunless %}{% endfor %}
</p>
...
{%- unless assigned contains needle -%}{%- assign unsorted = unsorted | plus: 1 -%}{%- endunless -%}
{%- endfor -%}
{% if unsorted > 0 %}
